...
import GrupedSortedExpanded from './grouped-and-sorted-chart-expanded';
...
export default class Chat extends Component {
  ...
  montChart() {
    ...
    const chart = am4core.create('grouped-sorted', am4charts.XYChart);
    ...
    for (const providerName in data) {
      ...
      range.label.adapter.add('maxWidth', function(maxWidth, target) {
        ...
        const startPosition = categoryAxis.categoryToPosition(
          range.category,
          0
        );
        const endPosition = categoryAxis.categoryToPosition(
          range.endCategory,
          1
        );
    ...
    return (
      <>
        <div className="section-line">
          <span className="title">ITENS POR DIA</span>
          <button className="line-button" onClick={this.handleShow}>
            +
          </button>
          <div id="grouped-sorted" />
        </div>
