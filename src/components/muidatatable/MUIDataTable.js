...
class MUIDataTable extends React.Component {
  ...
  setTableData(props, status, dataUpdated, callback = () => {}) {
    let tableData = [];
    const { columns, filterData, filterList } = this.buildColumns(
      props.columns,
      this.state.columns
    );
    let sortIndex = null;
  ...
  computeDisplayRow(
    ...
    for (let index = 0; index < row.length; index++) {
      ...
      if (
        ...
        // Quero procurar mesmo em colunas escondidas
  ...
  toggleExpandRow = row => {
    ...
    } else if (isRowExpandable && isRowExpandable(dataIndex, expandedRows))
  ...
  sortTable(data, col, order) {
    const dataSrc = this.options.customSort
